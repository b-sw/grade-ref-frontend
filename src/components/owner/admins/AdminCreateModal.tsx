...
export const AdminCreateModal = (props: Props) => {
    ...
    const createAdmin = (values: FormikValues) => {
        postMutation.mutate({
            email: values.email,
    ...
    return (
        <Modal isOpen={props.isOpen} onClose={props.onClose} isCentered>
            ...
            <ModalContent>
                ...
                <Formik initialValues={initialValues} onSubmit={createAdmin} validationSchema={userValidationSchema}>
                    {({ handleSubmit }) => (
                        <Form onSubmit={handleSubmit}>
                            <ModalBody>
                                <InputControl
                                    name="email"
                                    label="Email address"
                                    inputProps={{ placeholder: 'john.doe@gmail.com' }}
                                />
                                <InputControl
                                    name="phoneNumber"
                                    label="Phone number"
                                    inputProps={{ placeholder: '+48 669 797 907' }}
                                />
                                <InputControl
                                    name="firstName"
                                    label="First name"
